      ...
      <section className="py-20 bg-muted">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div className="text-center">
              <div className="w-20 h-20 bg-primary rounded-full flex items-center justify-center mx-auto mb-4">
                <FaHospital className="w-10 h-10 text-primary-foreground" />
              </div>
              <div className="text-3xl font-bold text-primary mb-2">
                {siteData.beds.replace('{{PLACEHOLDER_BEDS: ', '').replace('}}', '')}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="flex justify-between items-center py-2 border-b border-border">
                  ...
                  <span className="text-muted-foreground">{siteData.openingHours.weekdays}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-border">
                  <span className="font-medium text-card-foreground">Saturday</span>
                  <span className="text-muted-foreground">{siteData.openingHours.saturday}</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="font-medium text-card-foreground">Sunday</span>
                  <span className="text-muted-foreground">{siteData.openingHours.sunday}</span>
                </div>
              </div>
              
              <div className="md:border-l md:border-border md:pl-6">
                <div className="bg-emergency rounded-lg p-4 text-center">
                  <FaClock className="w-8 h-8 text-emergency-foreground mx-auto mb-2" />
                  <h3 className="font-bold text-emergency-foreground mb-1">Emergency Care</h3>
                  <p className="text-emergency-foreground text-sm">{siteData.openingHours.emergency}</p>
                  <a 
                    href={`tel:${siteData.phones.mobile}`}
                    className="inline-block mt-3 bg-emergency-foreground text-emergency font-medium px-4 py-2 rounded-lg hover:bg-emergency-foreground/90 transition-colors"
                  >
