import { siteData } from '@/data/siteData';
import { FaAward, FaClock, FaHospital, FaUsers } from 'react-icons/fa';
import { useI18n } from '@/i18n/I18nProvider';
import vedios from '/vedios/about-bg.mp4';

const About = () => {
  const { t } = useI18n();
  return (
   <div className="py-20">
  {/* Page Header */}
  <section className="relative bg-secondary py-16 overflow-hidden">
  {/* Background Video */}
  <video
    className="absolute inset-0 w-full h-full object-cover opacity-60"
    autoPlay
    loop
    muted
    playsInline
  >
    <source src={vedios} type="video/mp4" />
  </video>

  {/* Content Overlay */}
  <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 text-center">
    <h1 className="text-4xl lg:text-5xl font-bold text-foreground mb-6 font-serif">
      {t('about.title', { name: siteData.name })}
    </h1>

    <p className="text-xl text-secondary-foreground max-w-4xl mx-auto">
      {siteData.about.description}
    </p>
  </div>
</section>

  {/* Mission & Vision */}
  <section className="py-20">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 text-center">
        <div className="bg-card border border-border rounded-xl p-8">
          <div className="w-16 h-16 bg-primary rounded-lg flex items-center justify-center mx-auto mb-6">
            <FaHospital className="w-8 h-8 text-primary-foreground" />
          </div>
          <h2 className="text-2xl font-bold text-card-foreground mb-4 drop-shadow-md">
            {t('about.ourVision')}
          </h2>
          <p className="text-muted-foreground leading-relaxed italic text-center">
            {siteData.about.vision}
          </p>
        </div>

        <div className="bg-card border border-border rounded-xl p-8">
          <div className="w-16 h-16 bg-primary rounded-lg flex items-center justify-center mx-auto mb-6">
            <FaUsers className="w-8 h-8 text-primary-foreground" />
          </div>
          <h2 className="text-2xl font-bold text-card-foreground mb-4 drop-shadow-md">
            {t('about.ourMission')}
          </h2>
          <p className="text-muted-foreground leading-relaxed italic text-center">
            {siteData.about.mission}
          </p>
        </div>
      </div>
    </div>
  </section>



      {/* Hospital Facts */}
<section className="py-20 bg-muted">
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div className="text-center mb-16">
      <h2 className="text-3xl lg:text-4xl font-bold text-foreground mb-4 font-serif">
        {t('about.hospitalOverview')}
      </h2>
      <p className="text-xl text-muted-foreground">
        {t('about.keyFacts')}
      </p>
    </div>

    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 justify-items-center">
      <div className="text-center">
        <div className="w-20 h-20 bg-primary rounded-full flex items-center justify-center mx-auto mb-4">
          <FaUsers className="w-10 h-10 text-primary-foreground" />
        </div>
        <div className="text-3xl font-bold text-primary mb-2">{siteData.patientsTreated}</div>
        <div className="text-muted-foreground">{t('about.patientsTreated')}</div>
      </div>

      <div className="text-center">
        <div className="w-20 h-20 bg-primary rounded-full flex items-center justify-center mx-auto mb-4">
          <FaClock className="w-10 h-10 text-primary-foreground" />
        </div>
        <div className="text-3xl font-bold text-primary mb-2">{siteData.years}</div>
        <div className="text-muted-foreground">{t('about.yearsService')}</div>
      </div>

      <div className="text-center">
        <div className="w-20 h-20 bg-primary rounded-full flex items-center justify-center mx-auto mb-4">
          <FaAward className="w-10 h-10 text-primary-foreground" />
        </div>
        <div className="text-3xl font-bold text-primary mb-2">30+</div>
        <div className="text-muted-foreground">{t('about.treatmentTypes')}</div>
      </div>
    </div>
  </div>
</section>


   {/* Awards & Certifications */}
<section className="py-20">
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div className="text-center mb-16">
      <h2 className="text-3xl lg:text-4xl font-bold text-foreground mb-4 font-serif">
        {t('about.awardsCerts')}
      </h2>
      <p className="text-xl text-muted-foreground">
        Recognition for excellence in dermatological care
      </p>
    </div>

    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 place-items-center">
      {siteData.awards.map((award, index) => (
        <div
          key={index}
          className="bg-card border border-border rounded-xl p-6 text-center card-hover flex flex-col justify-center items-center w-full max-w-sm"
        >
          <div className="w-16 h-16 bg-primary rounded-full flex items-center justify-center mb-4">
            <FaAward className="w-8 h-8 text-primary-foreground" />
          </div>
          <h3 className="font-semibold text-card-foreground mb-2">
            {award.replace(/\{\{PLACEHOLDER_AWARD_\d+: /, '').replace('}}', '')}
          </h3>
        </div>
      ))}
    </div>
  </div>
</section>


      {/* Opening Hours */}
      {/* Opening Hours */}
<section className="py-20 bg-secondary">
  <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
    <h2 className="text-3xl lg:text-4xl font-bold text-foreground font-serif mb-8">
      {t('about.openingHours')}
    </h2>
    
    <div className="bg-card border border-border rounded-xl p-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="flex flex-col items-start py-2 border-b border-border">
            <span className="font-medium text-card-foreground text-lg">{t('about.mondaySaturdayOPD')}</span>
            <span className="text-muted-foreground text-sm">{siteData.openingHours.weekdays}</span>
          </div>
          
          <div className="flex flex-col items-start py-2 border-b border-border">
            <span className="font-medium text-card-foreground text-lg">{t('about.mondaySaturdayClinic')}</span>
            <span className="text-muted-foreground text-sm">{siteData.openingHours.clinic}</span>
          </div>

          <div className="flex flex-col items-start py-2">
            <span className="font-medium text-card-foreground text-lg">{t('about.sunday')}</span>
            <span className="text-muted-foreground text-sm">{siteData.openingHours.sunday}</span>
          </div>
        </div>
        
        <div className="md:border-l md:border-border md:pl-6">
  <div className="bg-blue-500 rounded-lg p-4 text-center"> {/* Changed bg-emergency to bg-blue-500 */}
    <FaClock className="w-8 h-8 text-white mx-auto mb-2" /> {/* Adjusted text color to white for contrast */}
    <h3 className="font-bold text-white mb-1">{t('about.emergencyCare')}</h3>
    <p className="text-white text-sm">{siteData.openingHours.emergency}</p>
    <a 
      href={`tel:${siteData.phones.mobile}`}
      className="inline-block mt-3 bg-white text-white-500 font-medium px-4 py-2 rounded-lg hover:bg-white/90 transition-colors"
    >
      {siteData.phones.mobile}
    </a>
  </div>
</div>
      </div>
    </div>
  </div>
</section>
    </div>
  );
};

export default About;