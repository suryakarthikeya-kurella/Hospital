      ...
      <section className="py-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-4 lg:px-8">
          <div className="relative overflow-hidden">
            ...
            >
              {CASES.map((c) => (
                <article key={c.id} className="snap-center min-w-[90%] sm:min-w-[520px] bg-card border border-border rounded-xl overflow-hidden shrink-0">
                  <div className="h-56 bg-muted overflow-hidden flex items-center justify-center">
                    <img src={c.image} alt={c.title} className="max-w-full max-h-full object-contain" loading="lazy" />
                  </div>
                  <div className="p-8">
                    <h3 className="text-xl font-semibold text-card-foreground mb-2">{c.title}</h3>
                    <h4 className="text-sm font-medium text-card-foreground mb-2">{t('cases.caseDescription')}</h4>
                    <p className="text-muted-foreground mb-4">{c.description}</p>
                    <h4 className="text-sm font-medium text-card-foreground mb-2">{t('cases.treatmentUsed')}</h4>
                    <p className="text-muted-foreground mb-4">{c.treatment}</p>
                    <h4 className="text-sm font-medium text-card-foreground mb-2">{t('cases.doctorsNote')}</h4>
                    <div className="bg-secondary rounded-lg p-4">
                      <p className="text-secondary-foreground italic">"{c.note}"</p>
                      <div className="mt-3 pt-3 border-t border-border">
                        <p className="text-xs font-medium text-secondary-foreground">- Dr. M. Bhargavi, MBBS, MD, DVL</p>
              </div>
            </div>
                  </div>
                </article>
